...
def train(args, rank=0, world_size=1, use_ddp=False):
    ...
    NUM_ACCUMULATION_STEPS = getattr(args, 'accumulation_steps', 1)  # Default to 1 if not specified
    if rank == 0:
        print(f"Using gradient accumulation with {NUM_ACCUMULATION_STEPS} steps")
    ...
    while should_keep_training:
        ...
        for i_batch, data_blob in enumerate(epoch_pbar):
            ...
            # Logger - accumulate metrics but don't reset until optimizer step
            if rank == 0:
                if valid.sum() > 0:
                    # Scale loss back for logging (since we normalized it)
                    actual_loss = loss.item() * NUM_ACCUMULATION_STEPS
                    avg_loss.update(actual_loss)
                ...
                epoch_pbar.set_postfix({
                    ...
                    'AccumStep': f'{(i_batch + 1) % NUM_ACCUMULATION_STEPS}/{NUM_ACCUMULATION_STEPS}'
                })
            
            # Update optimizer only when accumulation steps are complete or at the end of epoch
            if ((i_batch + 1) % NUM_ACCUMULATION_STEPS == 0) or (i_batch + 1 == len(train_loader)):
                ...
                # Log metrics only after optimizer step
                if rank == 0:
            ...
            # Break from inner loop if training should stop
